...
Greedy policy 使用符號 `^ v < >` 表示每格 Q-table 最大動作，`G` 表 gold，`B` 表 bomb。完整 policy 已寫在 CSV。整體觀察是：epsilon 太高會直接降低 exploitation 階段表現；alpha 太高會使估計對近期 episode 過度敏感；樂觀初始值能改善早期探索，但在此小型 deterministic grid 中長期差異有限。
