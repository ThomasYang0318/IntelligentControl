...
這張圖由 `scripts/run_breakout_dueling_smoke.py` 使用 Python `matplotlib` 根據 smoke-run CSV 結果產生。圖中每個點旁的數字是該點的 running reward，線上的 `final` 標籤是最後 running reward；右側 summary panel 列出 final running reward、last-5 mean、best reward、mean loss 與參數量。單集 episode reward 保留在 `docs/breakout_dueling_smoke_episodes.csv`。
